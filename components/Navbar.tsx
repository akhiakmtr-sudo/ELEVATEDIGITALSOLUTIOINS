...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3">
              <a href="#" className="hover:scale-110 transition-transform"><Facebook className="h-4 w-4 fill-current" /></a>
              <a href="#" className="hover:scale-110 transition-transform"><Instagram className="h-4 w-4" /></a>
              <a href="#" className="hover:scale-110 transition-transform"><Twitter className="h-4 w-4 fill-current" /></a>
              <a href="#" className="hover:scale-110 transition-transform"><Linkedin className="h-4 w-4 fill-current" /></a>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Main Navbar */}
      <nav className={`bg-white transition-all duration-300 ${scrolled ? 'shadow-md py-1' : 'py-0'} h-20 lg:h-24`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
          <div className="flex items-center justify-between h-full">
            ...
            <div className="hidden lg:flex items-center">
              <div className="flex items-center space-x-8 mr-12">
                {NAV_ITEMS.map((item) => (
                  <a
                    ...
                    className={`relative py-2 text-sm lg:text-[15px] font-bold transition-all duration-300 ${
                      activeTab === item.label ? 'text-[#0061ff]' : 'text-slate-700 hover:text-[#0061ff]'
                    }`}
                  >
                    {item.label}
                    <span className={`absolute bottom-0 left-0 w-full h-[3px] bg-[#0061ff] transform transition-transform duration-300 ${activeTab === item.label ? 'scale-x-100' : 'scale-x-0'}`}></span>
                  </a>
                ))}
              </div>
              ...
              <a 
                href="tel:6353539425"
                className="flex items-center gap-2 bg-[#0061ff] hover:bg-blue-700 text-white px-7 py-3.5 rounded-full font-bold text-sm lg:text-base transition-all duration-300 shadow-lg shadow-blue-200 active:scale-95"
              >
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="lg:hidden bg-white border-t border-slate-100 shadow-2xl animate-in slide-in-from-top duration-300 overflow-hidden">
